...
export class AbsenceService {
  ...
  post(absence: Absence): Observable<Absence> {
    return this.http.post<Absence>(url + 'Post', absence, {headers: new HttpHeaders().set('Authorization', 'Bearer ' + jwt)});
  }

  getAll(): Observable<Absence[]> {
    return this.http.get<Absence[]>(url + 'GetAll', {headers: new HttpHeaders().set('Authorization', 'Bearer ' + jwt)});
  }
  ...
  put(absence: Absence): Observable<Absence> {
    return this.http.put<Absence>(url + 'Put' + '/' + absence.Id, absence, {headers: new HttpHeaders().set('Authorization', 'Bearer ' + jwt)});
  }
